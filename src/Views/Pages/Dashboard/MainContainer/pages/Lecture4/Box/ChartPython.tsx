...
import { markdowns } from '@src/Data/markdowns/Lecture4/box.markdown';
...
const ChartPython = () => {
	...
	const getResult = async () => {
		try {
			...
			// ?countries=${countries_string}&age=${selections?.age}&sex=${selections?.sex}&
		...
		const getInputs = async () => {
			try {
				...
				const {
					...
					SEX: sex,
					RESPONSE: response,
				} = await Get({ url: `/lecture5/box-python-selections` });
