...
const ChartPresentation = () => {
	...
	const [states, setStates] = useState<string[]>(['Arizona', 'Massachusetts', 'Georgia']);
	...
	const getResult = async () => {
		try {
			...
			const plot = await fetch(`${CONFIG.SERVER}/lecture6/area-chart?states=${concat_selections}&part=3`).then(res => {
				return res.text();
			});
			...
			<div className='result'>
				<Grid className='result-grid animate-fade-in'>
					<Row className='flex items-center mb-8'>
						<Col className='justify-self-start' md={10}>
							<SourceCode code={source_codes?.PRESENTATION} contentClass='small-source-code' language={'python'} />
						</Col>
						<Col className='ml-auto' md={13}>
					...
					<Row>
						<Tutorial data={markdowns?.PRESENTATION} language='python' />
					</Row>
				</Grid>
			</div>
		</div>
