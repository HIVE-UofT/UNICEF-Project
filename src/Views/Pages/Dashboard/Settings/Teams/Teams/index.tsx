...
const Users = () => {
	...
	const getUsers = async () => {
		try {
			const { items } = (await TEAMS?.getItems()) || {};
	...
	const rightSideHeader = (
		<div className='right-header-search'>
			...
			<Btn fa='r-plus' onClick={() => push(`${location.pathname}/new`)} children='New Datathon' />
			</If>
		</div>
	);
	...
	return (
		<Container className='users-layout'>
			<Box {...{ header, rightSideHeader }} className='panel' headerSeparator={width < 1550 ? false : true}>
				<TableRenderer {...{ cols }} {...registerTable} data={filteredData} loading={TEAMS?.loadings?.getItems} />
		...
		Cell: ({ row: { original } }: any) => {
			...
			return (
				<FaIcon
					...
					onClick={() => push(`${location.pathname}/${original?.id}#edit`)}
				/>
			);
		},
	},
